...
- **Snapshots** (`Snapshots/`, 2026-09-11; ARCHITECTURE.md, "Snapshots") — a
  ...
  - `SnapshotPreferences` — the snapshots folder, machine-local, defaulting to
    ...
    folder.
  - `SnapshotListing` — the images directly in the folder, newest first, not
    filtered by name: the one reading the Library's Snapshots tab lists and
    the Data pane counts.
...
- `LibraryView` — the Library pane (GLOSSARY.md), the trailing sidebar's
  lower pane: a **Library** heading with a Media | Snapshots segmented
  control. The **Media** tab lists the project's media files, with the Add
  ...
  file as media.
- `LibraryTab` — the Library's tabs, Media and Snapshots, persisted in
  `LibraryPreferences`.
- `LibraryList` — the one file list every Library tab draws: a Quick Look
  ...
  media row as its input (`DraggedInput`), a file row as its file — and
  dimmed with a warning when its file is missing.
...
- `LibraryItem` — one Library row as a value: its `Identity` — a media
  ...
  content type), modification date, size, a movie's duration, and whether its
  input is registered; with the pure detail-line rule (date — with the time
  for a file row — then duration or size) and the rows-from-media and
  rows-from-snapshots builders the tests exercise.

...
**Releases are cut in CI.** `.github/workflows/release-cli.yml` runs every step below on a manual dispatch — the version bump, the signed and notarized build, the tag, the GitHub release, and the tap push — so a release does not depend on one developer's Mac, keychain, or `gh` login. It is a thin wrapper over the same `scripts/release-cli.sh` described next, run with `--yes`, so the CI path and a hand-cut release are the same code. The workflow's header lists the repository secrets it needs. Two credentials differ from a local run: the tap push needs a PAT (`TINGRA_RELEASE_TOKEN`) because `GITHUB_TOKEN` cannot reach a second repository, and notarization uses an App Store Connect API key, which the job stores as a `notarytool` keychain profile in a throwaway keychain and points the packaging script at with `TINGRA_NOTARY_KEYCHAIN` — a runner has no login keychain profile to read.

...
**Status: approved 2026-08-15; amended 2026-08-15 after v0.1.1 — the shared keychain access group is reverted for `tingra-cli` (see "Key sharing between the app and the daemon"). Steps 2 and 3 of the sequencing are built; step 4 (the app adopting the store) is outstanding, and how the daemon reaches an app-filed key is reopened.** Each decision here lands in the doc that owns its area as it is built, per the decide-then-build rule: the store and the shared access group are in [TYPES.md](TYPES.md) and [README.md](../README.md), the tool surface in [MCP.md](MCP.md), and the two new error identifiers in [CLI.md](CLI.md). [GLOSSARY.md](GLOSSARY.md)'s Destination entry is rewritten with step 4.

...
**Manifest** — what an app-tier plug-in declares about itself, as one dictionary: its identifier, its name, and its pane, command, and settings pane descriptors. It lives in the extension's Info.plist as the `TingraPlugIn` dictionary under `EXAppExtensionAttributes`, beside the extension point identifier ExtensionKit reads, so the app registers a plug-in at discovery with no process launched — and a manifest that does not decode is reported as an error naming the extension and the key, never a silently missing menu.

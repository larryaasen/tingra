...
engine services & plug-ins
    ...
    EventBus (host)
        ...
        ├──►  File sink         attached by --log-file; always on in the app (~/Library/Logs/Tingra/Tingra.log)
        ├──►  Log window sink   the app's Log window, attached only while it is open and not paused
...
- **OSLog sink** — the system of record. Uses `os.Logger` with `subsystem` = `com.moonwink.tingra` and `category` = the event's **domain** (a 1:1 mapping). Group maps to level per the table above. Event names and groups interpolate as public; **params interpolate as `privacy: .private` by default**.
  ...
  **The app attaches it unconditionally (decided 2026-09-07).** Until then `apps/tingra-app` attached only its stdout `ConsoleEventSink`, so a Tingra.app launched from the Finder recorded nothing anywhere — the console sink's own comment called OSLog "always-on", but only the CLI ever attached it. The app never runs with a terminal on stderr, so the CLI's exception below does not apply to it; it attaches `OSLogSink` at boot beside the console sink and the file sink, and `shutDown` awaits all three drains.
...
- **File sink** — the same formatted lines as the console's human mode, byte for byte, appended to a file; every group, no filter. `tingra-cli` attaches it when `--log-file` is passed. **The app attaches it always, to one fixed file (decided 2026-09-07, Larry approved; the record is in ARCHITECTURE.md, "The log file and the Logging settings pane").** The sink moves from `tingra-cli` into `TingraHost` as public API with that work — two front ends needing the same sink is the exact trigger that moved `LogLineFormatter` there — and the CLI keeps using it unchanged.
  - **Where:** `~/Library/Logs/Tingra/Tingra.log`, the folder Mac apps put their logs in and the one Console.app's Log Reports lists. Not Documents (an iOS sandbox choice, where Auto Care Plus keeps its log) and not Application Support, which holds the show.
  - **Always on, every group, append-only, cleared by hand.** A support log that was switched off when the defect happened is useless, and the `network`/`trace` debug lines are what a developer reading a shared log wants. There is no rotation yet: the size after a real production session is measured first (`stream.stats` is the line that grows it), and a rollover is proposed only if that number says so — instrument before guessing (TODO.md).
  - **Clearing truncates in place**, never deletes: the sink opens a fresh handle per line and seeks to the end, so an emptied file keeps working with no window between a delete and the next write recreating it. The clear itself is recorded as one `log.cleared` event (domain `platform`, param `previousBytes`), so a short log read later says it was deliberately cleared rather than looking truncated by accident; a clear that could not complete is a `log.clear` error. Remove All Data (the Data settings pane) **keeps** the log file, listed the way recordings are: the removal and the quit that follows are exactly what a developer checking a first run by hand wants on record, and the sink would recreate the file on the very next event anyway.
  - **Sharing copies a snapshot.** Share Log File hands the share picker a dated copy (`Tingra Log 2026-09-08.txt`) rather than the live file being appended to, the Auto Care Plus rule — taken **at share time**: the pane's `ShareLink` carries a `Transferable` whose file representation runs the copy when the share happens, and the Help menu's item copies before showing the system picker. A snapshot that cannot be taken (no file, or an unwritable one) is a `log.snapshot` error carrying the reason.
  - **Params are written in the clear**, like the console sink and the CLI's `--log-file`: the file sink relies on Redaction's first layer — secrets never become params — and adds no second one, so the OSLog sink remains the only sink with privacy redaction of its own.
- **Log window sink** — the app's **Log** window reading the log file back (decided and built 2026-09-12, `LogWindowSink`; the record is in ARCHITECTURE.md, "The log window"). It keeps no events of its own: the window reads the file's last lines for history and attaches this sink only while it is open and not paused, formatting each event with the same `LogLineFormatter` so a live line matches the file's byte for byte, and cancelling the attach when the window closes or pauses. It is the reason a closed log window costs nothing — there is no in-memory ring filling up behind it.
- **The first line of every app log is `app.launched`** (group `app`, domain `platform`; decided 2026-09-07): the app version and build (`appVersion`, `appBuild`), the macOS version and build (`systemVersion` from `ProcessInfo`, `systemBuild` from `sysctl kern.osversion`), and the hardware model identifier (`hardwareModel`, from `sysctl hw.model`, e.g. `Mac15,3` — Apple exposes no marketing name); a reading that is missing drops its key rather than writing a placeholder (`LaunchDiagnostics` in the app). The Tingra counterpart of Auto Care Plus's `app.coldstart`/`SupportDiagnostics`: a shared log that does not start by identifying the build it came from cannot be lined up with a report. Emitted in `EngineModel.start()` before anything else, so the sinks attached a line earlier receive it first; until then the only `app` event the app emitted was `app.terminating`. No launch count for now.

### The human log line format

One line format is shared by every text sink — the CLI's console (human mode) and file sinks and the app's console and file sinks:

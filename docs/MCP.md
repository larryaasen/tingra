...
- A destination that refuses the connection at start does **not** fail `stream_start` while another goes live (the CLI's best-effort start, per CLI.md); the refusal is reported on the event bus and the leg reads as absent from `stream_status`. `stream_start` fails only when every destination is refused.

...
| `tingra/secrets.get`, `tingra/secrets.set` | extension → app | One of the plug-in's own secrets by `name` (`token`): read as `{value}` (a string, or `null` when none is stored), stored as a string `value`, removed with `null`. Filed in the engine's Keychain-backed secure storage under `plugin:<PlugInID>.<name>` — the plug-in id is the connection's, so a plug-in reaches its own secrets and never another's or a stream key. A store that refuses answers an internal error naming the operation and the store's reason, never the value (PLUGINS.md, "Phase 2 as built, fourth slice") |
...
| `tingra/statusItem.set` | extension → app | The text of one of the plug-in's declared status items: `item` (the id within the plug-in) and `text` (a string; `null` or empty takes the reading off the bar). The app draws the reading on the windows' status bar beside the item's declared symbol — no scene is hosted for it — and drops it when the plug-in's last connection closes. An item the manifest does not declare is invalid params (PLUGINS.md, "Phase 2 as built, fifth slice") |
...
**The stream and recording tools** (built 2026-09-18, PLUGINS.md Decision 17 and "Phase 2 as built, sixth slice"). Beside `shot_take`, `preview_set`, and `fade_to_black` the app registers `program_stream_start`, `program_stream_stop`, `program_record_start`, and `program_record_stop`. They take **no arguments** — the project's enabled destinations and the program are what they act on, the stream keys are read from the Keychain as the toolbar button's are, and the recording goes where the operator's settings say — so the daemon's `stream_*` schemas stay untouched and one tool name never carries two. Each names a state rather than a toggle: a start while on air, or a stop while off, answers the state with `changed: false` and is not an error. Results carry `tingra://session`'s own `stream` or `recording` state members (plus `changed`, and the recording's `path`); a start returns once the session is *starting*, and the resource reports what becomes of it. Errors: `destinationNotFound` when the project has nothing enabled with a usable URL, `pipelineError` or `recordingFailed` when a start settles on an error at once.

**Meters and frames** (built 2026-09-18, PLUGINS.md Decision 18 and "Phase 2 as built", seventh and eighth slices) are the two things a plug-in observes that are not resources, because each is a stream of values stale the moment it is read:

| Method | Direction | What it carries |
|---|---|---|
| `tingra/meters.subscribe`, `tingra/meters.unsubscribe` | extension → app, requests | Opt this connection in to, or out of, the meters. No params; `{}`. Subscribing twice is one subscription; it ends with the connection |
| `tingra/meters` | app → extension, notification | One **window** of the mix, at most ten a second and none while the mixer is stopped: `time` (the window's last mix tick, seconds on the master clock), `strips` (an object keyed by input id, each `{"peak", "rms"}`, pre-fader), and `master` (`left` and `right`, post-fader). Levels are linear sample magnitudes — `1` is full scale, a peak above it an over — a window's peak the largest of its blocks', its RMS the blocks' own |
| `tingra/frame.next` | extension → app, request | A demand for the next frame of a bus: `bus` (`program` or `preview`). Answered `{}` at once; one demand per bus is outstanding at a time, and asking again meanwhile changes nothing. An unknown bus is invalid params |
| `tingra/frame` | app → extension, notification | The frame a demand asked for, sent as soon as the bus has one newer than the last this connection was sent: `bus`, `time`, `empty` — and the frame's `IOSurface` **attached to the same XPC message beside the JSON** (`SurfaceMessageTransport`), shared memory, not a copy. `empty: true` carries no surface: the bus has nothing on it (preview with no shot staged) |

Neither is served by the daemon, which has no mixer and no compositor, and whose socket cannot carry a surface. The session's part is small and generic: a `SessionMethodHandler` is handed a `SessionNotifier` when its session opens (`sessionOpened(notifier:)`) and told when it closes, so an endpoint can send notifications of its own; the daemon's sessions have no handler and behave as before. The attachment is not a second protocol — the payload is still JSON-RPC and still names the method; the surface is the one value JSON cannot carry.

Identity is per connection: the app opened the connection for a known extension, so the handler is built with that plug-in's id. Secrets are not a storage scope: a plug-in's keys go through `tingra/secrets.*`, narrowed to its own Keychain items, never a plaintext file.

...
**Named destinations are a proposed addition** — the store, the `destinations_list` tool, and the `destination` selector on `stream_start`/`probe` that would let an agent resolve "my Twitch" by name — recorded and awaiting approval in [DESTINATIONS.md](DESTINATIONS.md).
